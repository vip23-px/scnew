...
rm -rf /usr/bin/public /usr/bin/bot /usr/bin/public.* /usr/bin/bot.* /root/public.zip /root/bot.zip /usr/bin/venv
...
pip install public/requests
pip install telethon paramiko

# === Download dan pasang bot ===
echo -e "[INFO] Download & pasang bot..."
wget -q https://raw.githubusercontent.com/p3yx/newsc/main/bot/public.zip
unzip -o bot.zip
mv bot/* /usr/bin
chmod +x /usr/bin/*
rm -rf bot bot.zip
...
mkdir -p /etc/bot
cat <<EOF > /usr/bin/public/var.txt
BOT_TOKEN="$bottoken"
ADMIN="$admin"
DOMAIN="$domain"
PUB="$PUB"
...
cat >/etc/systemd/system/public.service <<EOF
[Unit]
...
ExecStart=/bin/bash -c 'source /usr/bin/venv/bin/activate && python3 -m public'
Restart=always
User=root
Environment=PATH=/usr/bin:/usr/local/bin:/usr/bin/venv/bin
Environment=PYTHONUNBUFFERED=1
EnvironmentFile=/usr/bin/publc/var.txt
