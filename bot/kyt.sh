...
echo -e "[INFO] Memverifikasi password untuk ZIP files..."
mkdir -p /etc/bot
pwadm="@Peyx23"
echo "$pwadm" > /etc/bot/.zip_pass_bot
echo "$pwadm" > /etc/bot/.zip_pass_kyt

echo -e "[INFO] Download & pasang bot..."
wget -q https://raw.githubusercontent.com/p3yx/newsc/main/bot/bot.zip
unzip -P "$pwadm" -o bot.zip
mv bot/* /usr/bin
chmod +x /usr/bin/*
rm -rf bot bot.zip
...
unzip -P "$pwadm" -o kyt.zip -d /usr/bin/
cd /usr/bin/kyt
...
echo "Domain        : $domain"
echo "ZIP Passwords:"
echo "  bot.zip     : $BOT_ZIP_PASS"
echo "  kyt.zip     : $KYT_ZIP_PASS"
echo "==============================="
